"""Matrix Transpose 的 PyTorch CUDA Extension 包装层。"""
...
EXTENSION_DIR = Path(tempfile.gettempdir()) / "llm_kernel_lab_extensions" / "transpose"
...
def transpose_cuda_naive(x: torch.Tensor) -> torch.Tensor:
    """调用存在非合并写入的朴素 CUDA Transpose。"""
    return _load_extension().naive(x)


def transpose_cuda_tiled(x: torch.Tensor) -> torch.Tensor:
    """调用使用共享内存和 Padding 的 Tiled CUDA Transpose。"""
    return _load_extension().tiled(x)


def transpose_cuda_tiled_unpadded(x: torch.Tensor) -> torch.Tensor:
    """调用不使用 Padding、用于 Bank Conflict 对照的 Tiled CUDA Transpose。"""
    return _load_extension().tiled_unpadded(x)
